...
enum { MemoryAccessControlNormalOrder,
	MemoryAccessControlReverseOrder } MemoryAccessControlRefreshOrder;

enum { MemoryAccessControlColorOrderRGB,
	MemoryAccessControlColorOrderBGR } MemoryAccessControlColorOrder;
...
static void lcdDrawPixels(
    uint16_t x, uint16_t y, uint16_t *data, uint32_t dataLength);
...
static unsigned char lcdBuildMemoryAccessControlConfig(bool rowAddressOrder,
    bool columnAddressOrder, bool rowColumnExchange, bool verticalRefreshOrder,
    bool colorOrder, bool horizontalRefreshOrder);
 ...
 */
void
lcdDrawPixel(uint16_t x, uint16_t y, uint16_t color)
{
	// Clip
...
void
lcdDrawHLine(uint16_t x0, uint16_t x1, uint16_t y, uint16_t color)
{
...
void
lcdDrawVLine(uint16_t x, uint16_t y0, uint16_t y1, uint16_t color)
{
	if (y1 < y0) {
		swap(y0, y1);
	}
...
/**
 * \brief Draws a line connecting the two points specified by the coordinate
 * pairs
 *
 ...
 */
void
lcdDrawLine(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
...
/**
 * \brief Draws a rectangle specified by a coordinate pair, a width, and a
 * height.
 *
 * \param x			The x-coordinate of the upper-left corner of the
 * rectangle to draw
 * \param y			The y-coordinate of the upper-left corner of the
 * rectangle to draw \param w			Width of the rectangle to draw
 ...
 */
void
lcdDrawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
...
/**
 * \brief Draws a rectangle with rounded corners specified by a coordinate pair,
 ...
 */
void
lcdDrawCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
...
/**************************************************************************/
void
lcdDrawTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2,
 ...
 */
void
lcdFillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
 ...
 */
void
lcdFillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername,
...
/**
 * \brief Draws a filled rectangle specified by a coordinate pair, a width, and
 * a height.
 *
 * \param x				The x-coordinate of the upper-left corner of
 * the rectangle to draw
 * \param y				The y-coordinate of the upper-left corner of
 * the rectangle to draw \param w				Width of the
 * rectangle to draw
 * \param h				Height of the rectangle to draw
 ...
 */
void
lcdFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t fillcolor)
{
	// clipping
...
/**
 * \brief Draws a filled rounded rectangle specified by a coordinate pair, a
 * width, and a height.
 *
 * \param x				The x-coordinate of the upper-left corner of
 * the rectangle to draw
...
/**************************************************************************/
void
lcdFillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2,
    int16_t y2, uint16_t color)
{
...
void
lcdDrawImage(uint16_t x, uint16_t y, GUI_CONST_STORAGE GUI_BITMAP *pBitmap)
{
	if ((x >= lcdProperties.width) || (y >= lcdProperties.height))
		return;
 ...
 */
void
lcdDrawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg)
{
	...
	for (uint8_t i = 0; i < lcdFont.pFont->Height; i++) {
		...
		for (uint8_t k = 0; k < fontCoeff; k++) {
			line = lcdFont.pFont
				   ->table[((c - 0x20) * lcdFont.pFont->Height *
					       fontCoeff) +
				       (i * fontCoeff) + k];
...
lcdPrintf(const char *fmt, ...)
{
	...
	while (*p) {
		if (*p == '\n') {
			...
			lcdDrawChar(cursorXY.x, cursorXY.y, *p,
			    lcdFont.TextColor, lcdFont.BackColor);
 ...
 */
void
lcdSetTextColor(uint16_t c, uint16_t b)
{
	lcdFont.TextColor = c;
	lcdFont.BackColor = b;
}
 ...
 */
void
lcdSetTextWrap(uint8_t w)
{
	lcdFont.TextWrap = w;
}

void
lcdSetOrientation(lcdOrientationTypeDef value)
{
 ...
 */
uint16_t
lcdReadPixel(uint16_t x, uint16_t y)
{
	uint16_t temp[3];
	// Clip
	if ((x < 0) || (y < 0) || (x >= lcdProperties.width) ||
	...
	return lcdColor565(
	    (temp[1] >> 8) & 0xFF, temp[1] & 0xFF, (temp[2] >> 8) & 0xFF);
}

uint16_t
lcdColor565(uint8_t r, uint8_t g, uint8_t b)
{
...
static void
lcdDrawPixels(uint16_t x, uint16_t y, uint16_t *data, uint32_t dataLength)
{
	uint32_t i = 0;
...
static void
lcdWriteData(unsigned short data)
{
	LCD_DataWrite(data);
}
...
lcdBuildMemoryAccessControlConfig(bool rowAddressOrder, bool columnAddressOrder,
    bool rowColumnExchange, bool verticalRefreshOrder, bool colorOrder,
